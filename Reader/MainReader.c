...
learn_int mainReader(learn_int argc, learn_char** argv) {

	/* Create source input buffer */
	learn_char* program = argv[0];
	learn_char* input = argv[2];
	learn_char mode = MODE_FIXED;
	learn_int size = 0, increment = 0, wrongNumber = 0;
...
************************************************************
*/
learn_void startReader(learn_char* program, learn_char* input, learn_char mode, learn_int size, learn_int increment) {

	ReaderPointer bufferp;		/* pointer to Buffer structure */
	FILE* fileHandler;			/* input file handle */
	learn_int loadSize = 0;		/* the size of the file loaded in the buffer */
	learn_char symbol;			/* symbol read from input file */
...
learn_void bErrorPrint(learn_char* fmt, ...) {
	...
	(learn_void)vfprintf(stderr, fmt, ap);
	va_end(ap);
